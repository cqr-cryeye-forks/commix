...
def table_name(url):
    ...
    table_name = "session_" + host.replace(".", "_").replace(":", "_").replace("-", "_").replace("[", "_").replace("]",
                                                                                                                   "_")
    return table_name
...
def injection_point_importation(url, technique, injection_type, separator, shell, vuln_parameter, prefix, suffix, TAG,
                                alter_shell, payload, http_request_method, url_time_response, timesec, how_long,
        ...
        if settings.LOAD_SESSION == True:
            ...
            while True:
                if not menu.options.batch:
                    ...
                    settings.LOAD_SESSION = input(settings.print_question_msg(question_msg))
                else:
        ...
        if not menu.options.flush_session:
            ...
            if settings.TESTABLE_PARAMETER:
                cursor = conn.execute("SELECT output FROM " + table_name(url) + \
                                      "_ir WHERE cmd='" + base64.b64encode(
                    cmd.encode(settings.UNICODE_ENCODING)).decode() + "' AND " \
