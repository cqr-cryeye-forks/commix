...
def create_github_issue(err_msg, exc_msg):
    ...
    while True:
        try:
            if not menu.options.batch:
                ...
                choise = input(settings.print_question_msg(question_msg))
            else:
                choise = ""
            if len(choise) == 0:
                choise = "n"
    ...
    request = _urllib.request.Request(url="https://api.github.com/repos/commixproject/commix/issues",
                                      ...
                                      headers={"Authorization": "token " + base64.b64decode(
    ...
    issue_url = re.search(r"https://github.com/commixproject/commix/issues/\d+",
