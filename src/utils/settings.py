    ...
    for i in xrange(len(sys.argv)):
        ...
        if sys.argv[i].startswith("--tamper"):
            if tamper_index is None:
                tamper_index = i if '=' in sys.argv[i] else (
                    i + 1 if i + 1 < len(sys.argv) and not sys.argv[i + 1].startswith('-') else None)
            else:
                sys.argv[tamper_index] = "%s,%s" % (sys.argv[tamper_index],
                                                    sys.argv[i].split('=')[1] if '=' in sys.argv[i] else (
                                                        sys.argv[i + 1] if i + 1 < len(sys.argv) and not sys.argv[
                                                            i + 1].startswith('-') else ""))
                sys.argv[i] = ""
...
def sys_argv_errors():
    ...
    for i in xrange(len(sys.argv)):
        ...
        if len(sys.argv[i]) > 1 and all(ord(_) in xrange(0x2018, 0x2020) for _ in
                                        ((sys.argv[i].split('=', 1)[-1].strip() or ' ')[0], sys.argv[i][-1])):
...
SUFFIXES = ["", "'", "\"", "&&", "%26%26", "|", "%7C", "||", "%7C%7C", " #", "//", "\\\\", "%26'", "%27", "%22",
            "%5C%5C", "%2F%2F"]
...
EVAL_SYS_USERS = r"awk -F ':' '{print \$1}{print \$3}{print \$6}' " + PASSWD_FILE
...
CRAWL_EXCLUDE_EXTENSIONS = [
    ...
    "asf", "au", "avi", "bak", "bin", "bk", "bmp", "btif", "bz2", "cab", "caf", "cgm", "cmx", "cpio", "cr2", "dat",
    "deb",
    ...
    "weba",
    "webm", "webp", "whl", "wm", "wma", "wmv", "wmx", "woff", "woff2", "wvx", "xbm", "xif", "xls", "xlsx", "xlt", "xm",
    "xpi",
...
LINUX_DEFAULT_DOC_ROOTS = ["/var/www/", "/var/www/html", "/var/www/htdocs", "/usr/local/apache2/htdocs",
                           "/usr/local/www/data", "/var/apache2/htdocs", "/var/www/nginx-default",
                           "/srv/www/htdocs"]  # Reference: https://wiki.apache.org/httpd/DistrosDefaultLayout
