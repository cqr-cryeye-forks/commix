...
from src.thirdparty.colorama import Back, Style
...
def installer():
    ...
    if platform.system() == "Linux":
        ...
        if os.path.isdir("/usr/share/" + settings.APPLICATION + ""):
            ...
            while True:
                if not menu.options.batch:
                    ...
                    uninstall = input(settings.print_question_msg(question_msg))
                else:
                    uninstall = ""
                if len(uninstall) == 0:
                    uninstall = "Y"
        ...
        try:
            ...
            subprocess.Popen("cp -r " + current_dir + " /usr/share/" + settings.APPLICATION + " >/dev/null 2>&1",
                             shell=True).wait()
            subprocess.Popen(
                "chmod 775 /usr/share/" + settings.APPLICATION + "/" + settings.APPLICATION + ".py >/dev/null 2>&1",
                shell=True).wait()
        except Exception:
            print(settings.SINGLE_WHITESPACE)
            raise SystemExit()
