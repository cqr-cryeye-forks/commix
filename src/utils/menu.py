...
def banner():
    ...
    /\\ \\__//\\ \\/\\ \\/\\ \\/\\ \\/\\ \\/\\ \\/\\ \\/\\ \\ \\\\\\/>  </  
    \\ \\____\\ \\____/\\ \\_\\ \\_\\ \\_\\ \\_\\ \\_\\ \\_\\ \\\\_/\\_\\\\ {Fore.GREY}{Style.UNDERLINE}{settings.APPLICATION_URL}{Style.RESET_ALL}
    ...
    Copyright © {settings.YEAR} {settings.AUTHOR} ({Fore.LIGHTRED_EX}{settings.AUTHOR_TWITTER}{Style.RESET_ALL})
    +--
    """)
...
target.add_option("--crawl",
                  ...
                  dest="crawldepth",
                  type="int",
                  help="Crawl the website starting from the target URL (1-2, Default: " + str(
                      settings.DEFAULT_CRAWLDEPTH_LEVEL) + ").")
...
# request.add_option("-o", "--output-file",
#                    action="store",
#                    dest="output_file",
...
injection.add_option("--maxlen",
                     ...
                     help="Set the max length of output for time-related injection techniques (Default: " + str(
                         settings.MAXLEN) + " chars).")
...
def mobile_user_agents():
    print(
        """---[ """ + Style.BRIGHT + Fore.BLUE + """Available smartphones HTTP User-Agent headers""" + Style.RESET_ALL + """ ]---     
...
Type '""" + Style.BRIGHT + """11""" + Style.RESET_ALL + """' for Xiaomi Mi 3.""")
    ...
    while True:
        ...
        mobile_user_agent = input(settings.print_question_msg(question_msg))
        try:
