...
def timebased_command_injection_technique(url, timesec, filename, http_request_method, url_time_response):
    ...
    if not settings.SKIP_COMMAND_INJECTIONS:
        if (len(menu.options.tech) == 0 or "t" in menu.options.tech):
            ...
            if tb_handler.exploitation(url, timesec, filename, http_request_method, url_time_response, injection_type,
                                       technique) != False:
...
def filebased_command_injection_technique(url, timesec, filename, http_request_method, url_time_response):
    ...
    if not settings.SKIP_COMMAND_INJECTIONS:
        if (len(menu.options.tech) == 0 or "f" in menu.options.tech):
            ...
            if fb_handler.exploitation(url, timesec, filename, http_request_method, url_time_response, injection_type,
                                       technique) != False:
...
def perform_checks(url, http_request_method, filename):
    ...
    if menu.options.auth_url and menu.options.auth_data:
        ...
        if _urllib.request.urlopen(url, timeout=settings.TIMEOUT).read() == _urllib.request.urlopen(
                menu.options.auth_url, timeout=settings.TIMEOUT).read():
            ...
            while int(scan_level) < int(settings.HTTP_HEADER_INJECTION_LEVEL) and settings.LOAD_SESSION != True:
                while True:
                    if not menu.options.batch:
                        ...
                        next_level = input(settings.print_question_msg(question_msg))
                    else:
                        next_level = ""
                    if len(next_level) == 0:
                        next_level = "Y"
                    if next_level in settings.CHOICE_YES:
                        ...
                        if perform_checks(url, http_request_method,
                                          filename) == False and scan_level < settings.HTTP_HEADER_INJECTION_LEVEL:
