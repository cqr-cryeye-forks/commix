...
from src.thirdparty.colorama import Fore, Back, Style
...
def value_boundaries(value):
    if not menu.options.batch:
        ...
        procced_option = input(settings.print_question_msg(question_msg))
    else:
        procced_option = ""
    ...
    try:
        if action == "encode" and type(page) == str:
            ...
            try:
                ...
                print('Trying latin decode...')
                settings.UNICODE_ENCODING = 'latin'
                return page.decode(settings.UNICODE_ENCODING)
    except (UnicodeEncodeError, UnicodeDecodeError) as err:
...
def next_attack_vector(technique, go_back):
    while True:
        if not menu.options.batch:
            ...
            next_attack_vector = input(settings.print_question_msg(question_msg))
        else:
            next_attack_vector = ""
...
def procced_with_file_based_technique():
    while True:
        if not menu.options.batch:
            ...
            enable_fb = input(settings.print_question_msg(question_msg))
        else:
            enable_fb = ""
        if len(enable_fb) == 0:
            enable_fb = "Y"
    ...
    try:
        while True:
            if not menu.options.batch:
                ...
                continue_tests = input(settings.print_question_msg(question_msg))
            else:
                continue_tests = ""
    ...
    if settings.PS_ENABLED == None and menu.options.is_admin or menu.options.users or menu.options.passwords:
        ...
        while True:
            if not menu.options.batch:
                ...
                ps_check = input(settings.print_question_msg(question_msg))
            else:
                ps_check = ""
            if len(ps_check) == 0:
                ps_check = "Y"
        ...
        if cgi_script in url and menu.options.shellshock == False:
            ...
            while True:
                if not menu.options.batch:
                    ...
                    shellshock_check = input(settings.print_question_msg(question_msg))
                else:
                    shellshock_check = ""
            ...
            if re.search(r'^(?:http)s?://', url, re.I):
                if not re.search(r"^https?://", url, re.I) and not re.search(r"^wss?://", url, re.I):
                    if re.search(r":443\b", url):
                        ...
                        url = "http://" + url
                settings.SCHEME = (_urllib.parse.urlparse(
                    url).scheme.lower() or "http") if not menu.options.force_ssl else "https"
...
def tamper_scripts():
    if menu.options.tamper:
        ...
        if len(provided_scripts) >= 3 and not settings.LOAD_SESSION:
            ...
            _ = True
        elif len([x for x in provided_scripts if
                  any(y in x for y in ["nested", "doublequotes"])]) == 2 and not settings.LOAD_SESSION:
            _ = True
...
def process_xml_data():
    while True:
        ...
        if not menu.options.batch:
            ...
            xml_process = input(settings.print_question_msg(question_msg))
        else:
            if settings.VERBOSITY_LEVEL != 0:
                print(settings.print_bold_info_msg(info_msg))
            xml_process = ""
...
def process_json_data():
    while True:
        ...
        if not menu.options.batch:
            question_msg = info_msg
            question_msg += " Do you want to process it? [Y/n] > "
            json_process = input(settings.print_question_msg(question_msg))
        else:
            if settings.VERBOSITY_LEVEL != 0:
                print(settings.print_bold_info_msg(info_msg))
            json_process = ""
        ...
        for param in range(0, len(all_params)):
            if settings.IS_XML:
                if re.findall(r'</(.*)>', all_params[param]) == re.findall(r'>(.*)</', all_params[param]):
                    ...
                    all_params[
                        param] = "<" + parameter_name + ">" + parameter_name + settings.RANDOM_TAG + "</" + parameter_name + ">"
            else:
                if re.findall(r'(.*)=', all_params[param]) == re.findall(r'=(.*)', all_params[param]):
                    parameter_name = re.findall(r'=(.*)', all_params[param])
                    parameter_name = ''.join(parameter_name)
...
def generate_char_pool(num_of_chars):
    if menu.options.charset:
        ...
        char_pool = char_pool + list(range(49, 57)) + list(range(32, 48)) + list(range(91, 95)) + list(
            range(58, 64)) + list(range(123, 127))
    return char_pool
    ...
    if not re.match(settings.VALID_URL_FORMAT, menu.options.file_upload):
        ...
        while True:
            if not menu.options.batch:
                ...
                enable_HTTP_server = input(settings.print_question_msg(question_msg))
            else:
                enable_HTTP_server = ""
            ...
            if enable_HTTP_server in settings.CHOICE_YES:
                ...
                if settings.LOCAL_HTTP_IP == None:
                    while True:
                        ...
                        ip_addr = input(settings.print_question_msg(question_msg))
    ...
    if settings.TARGET_OS == "win" and menu.options.alter_shell:
        while True:
            if not menu.options.batch:
                ...
                python_dir = input(settings.print_question_msg(question_msg))
            else:
                python_dir = ""
            if len(python_dir) == 0:
                python_dir = "Y"
            if python_dir in settings.CHOICE_YES:
                ...
                settings.WIN_PYTHON_DIR = input(settings.print_question_msg(question_msg))
                break
            else:
