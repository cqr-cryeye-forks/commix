...
def powershell_version(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec, http_request_method, url,
                       vuln_parameter, alter_shell, filename, url_time_response):
    _ = False
    ...
    if session_handler.export_stored_cmd(url, cmd, vuln_parameter) == None or menu.options.ignore_session:
        ...
        check_how_long, output = tb_injector.injection(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec,
...
def hostname(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec, http_request_method, url, vuln_parameter,
...
def system_information(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec, http_request_method, url,
                       vuln_parameter, alter_shell, filename, url_time_response):
    _ = False
    ...
    if target_os:
        if settings.TARGET_OS != "win":
            ...
            if session_handler.export_stored_cmd(url, cmd, vuln_parameter) == None or menu.options.ignore_session:
                if settings.VERBOSITY_LEVEL == 0 and _:
                    sys.stdout.write("")
                check_how_long, output = tb_injector.injection(separator, maxlen, TAG, cmd, prefix, suffix, whitespace,
...
def current_user(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec, http_request_method, url,
                 vuln_parameter, alter_shell, filename, url_time_response):
    _ = False
    if settings.TARGET_OS == "win":
        ...
        if menu.options.is_root or menu.options.is_admin:
            ...
            if shell:
                ...
                if (settings.TARGET_OS == "win" and not "Admin" in shell) or \
                    ...
                    sys.stdout.write(
                        Style.BRIGHT + " and it is " + "not" + Style.RESET_ALL + Style.BRIGHT + " privileged" + Style.RESET_ALL + ".")
                    sys.stdout.flush()
                    # Add infos to logs file.
                    output_file = open(filename, "a")
                    ...
                    sys.stdout.write(
                        Style.BRIGHT + " and it is " + Style.RESET_ALL + Style.BRIGHT + "privileged" + Style.RESET_ALL + ".")
                    sys.stdout.flush()
                    # Add infos to logs file.
                    output_file = open(filename, "a")
...
def system_users(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec, http_request_method, url,
                 vuln_parameter, alter_shell, filename, url_time_response):
    _ = False
    if settings.TARGET_OS == "win":
            ...
            if sys_users[0]:
                ...
                for user in range(0, len(sys_users_list)):
                    ...
                    if menu.options.privileges:
                        ...
                        if alter_shell:
                            cmd = cmd.replace("'", "\\'")
                        check_how_long, output = tb_injector.injection(separator, maxlen, TAG, cmd, prefix, suffix,
                    ...
                    if settings.VERBOSITY_LEVEL != 0 and menu.options.ignore_session:
                        print(settings.SINGLE_WHITESPACE)
                    print("\n  [" + str(count) + "] '" + Style.BRIGHT + sys_users_list[
            ...
            if sys_users[0]:
                ...
                if len(sys_users) % 3 != 0:
                    ...
                    if len(sys_users_list) != 0:
                        ...
                        # Add infos to logs file.
                        output_file = open(filename, "a")
                        output_file.write(re.compile(re.compile(settings.ANSI_COLOR_REMOVAL)).sub("",
                        ...
                        for user in range(0, len(sys_users_list)):
                            ...
                            try:
                                ...
                                if menu.options.privileges:
                                    ...
                                    is_privileged = ""
                                    is_privileged_nh = ""
                                sys.stdout.write("\n    (" + str(count) + ") '" + Style.BRIGHT + fields[
                                    0] + Style.RESET_ALL + "'" + Style.BRIGHT + is_privileged + Style.RESET_ALL + "(uid=" +
                                                 fields[1] + "). Home directory is in '" + Style.BRIGHT + fields[
                                                     2] + Style.RESET_ALL + "'.")
                                sys.stdout.flush()
                                # Add infos to logs file.
                                output_file = open(filename, "a")
                                output_file.write(
                                    "    (" + str(count) + ") '" + fields[0] + "'" + is_privileged_nh + "(uid=" +
                                    fields[1] + "). Home directory is in '" + fields[2] + "'.\n")
                                output_file.close()
                            except ValueError:
                                if count == 1:
...
def system_passwords(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec, http_request_method, url,
                     vuln_parameter, alter_shell, filename, url_time_response):
    _ = False
    if settings.TARGET_OS == "win":
        # Not yet implemented!
                        ...
                        if ":" in line:
                            ...
                            if not "*" in fields[1] and not "!" in fields[1] and fields[1] != "":
                                print("  [" + str(count) + "] " + Style.BRIGHT + fields[
                                    0] + Style.RESET_ALL + " : " + Style.BRIGHT + fields[1] + Style.RESET_ALL)
                                # Add infos to logs file.
                                output_file = open(filename, "a")
...
def single_os_cmd_exec(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec, http_request_method, url,
                       vuln_parameter, alter_shell, filename, url_time_response):
...
def do_check(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec, http_request_method, url, vuln_parameter,
             alter_shell, filename, url_time_response):
    if menu.options.ps_version and settings.PS_ENABLED == None:
        if not checks.ps_incompatible_os():
            powershell_version(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec, http_request_method,
                               url, vuln_parameter, alter_shell, filename, url_time_response)
            settings.ENUMERATION_DONE = True

    if menu.options.hostname:
        if settings.ENUMERATION_DONE == True:
            print(settings.SINGLE_WHITESPACE)
        hostname(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec, http_request_method, url,
                 vuln_parameter, alter_shell, filename, url_time_response)
        settings.ENUMERATION_DONE = True
    ...
    if menu.options.current_user:
        if settings.ENUMERATION_DONE == True:
            print(settings.SINGLE_WHITESPACE)
        current_user(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec, http_request_method, url,
                     vuln_parameter, alter_shell, filename, url_time_response)
    ...
    if menu.options.sys_info:
        if settings.ENUMERATION_DONE == True:
            print(settings.SINGLE_WHITESPACE)
        system_information(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec, http_request_method, url,
                           vuln_parameter, alter_shell, filename, url_time_response)
        if settings.ENUMERATION_DONE == False:
            settings.ENUMERATION_DONE = True
    ...
    if menu.options.users:
        if settings.ENUMERATION_DONE == True:
            print(settings.SINGLE_WHITESPACE)
        system_users(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec, http_request_method, url,
                     vuln_parameter, alter_shell, filename, url_time_response)
        if settings.ENUMERATION_DONE == False:
            settings.ENUMERATION_DONE = True
    ...
    if menu.options.passwords:
        if settings.ENUMERATION_DONE == True:
            print(settings.SINGLE_WHITESPACE)
        system_passwords(separator, maxlen, TAG, cmd, prefix, suffix, whitespace, timesec, http_request_method, url,
                         vuln_parameter, alter_shell, filename, url_time_response)
        if settings.ENUMERATION_DONE == False:
            settings.ENUMERATION_DONE = True
