...
def system_information(separator, TAG, prefix, suffix, whitespace, http_request_method, url, vuln_parameter,
    ...
    if target_os:
        ...
        if settings.TARGET_OS != "win":
            ...
            if session_handler.export_stored_cmd(url, cmd, vuln_parameter) == None or menu.options.ignore_session:
                # Command execution results.
                response = cb_injector.injection(separator, TAG, cmd, prefix, suffix, whitespace, http_request_method,
                ...
                for user in range(0, len(sys_users_list)):
                    ...
                    if menu.options.privileges:
                        ...
                        if alter_shell:
                            cmd = cmd.replace("'", "\\'")
                        cmd = "cmd /c " + cmd
                        response = cb_injector.injection(separator, TAG, cmd, prefix, suffix, whitespace,
