...
def file_access(url, cve, check_header, filename):
    ...
    if menu.options.file_write:
        ...
        if os.path.split(menu.options.file_dest)[1] == "":
            ...
            dest_to_write = "/" + os.path.split(menu.options.file_dest)[1] + "/" + \
                            os.path.split(menu.options.file_write)[1]
        else:
...
def bind_tcp_config(url, cmd, cve, check_header, filename, os_shell_option, http_request_method, go_back,
                    go_back_again):
    ...
    if settings.BIND_TCP == False:
        if settings.REVERSE_TCP == True:
            ...
            reverse_tcp_config(url, cmd, cve, check_header, filename, os_shell_option, http_request_method, go_back,
                               go_back_again)
...
def reverse_tcp_config(url, cmd, cve, check_header, filename, os_shell_option, http_request_method, go_back,
                       go_back_again):
    ...
    if settings.REVERSE_TCP == False:
        if settings.BIND_TCP == True:
            ...
            bind_tcp_config(url, cmd, cve, check_header, filename, os_shell_option, http_request_method, go_back,
                            go_back_again)
        return go_back, go_back_again
...
def check_options(url, cmd, cve, check_header, filename, os_shell_option, http_request_method, go_back, go_back_again):
    ...
    # The "bind_tcp" option
        ...
        go_back, go_back_again = bind_tcp_config(url, cmd, cve, check_header, filename, os_shell_option,
                                                 http_request_method, go_back, go_back_again)
        return go_back, go_back_again

    # The "reverse_tcp" option
    elif os_shell_option == "reverse_tcp":
        go_back, go_back_again = reverse_tcp_config(url, cmd, cve, check_header, filename, os_shell_option,
                                                    http_request_method, go_back, go_back_again)
        return go_back, go_back_again
        ...
        for cve in shellshock_cves:
            for check_header in headers:
                ...
                if no_result == False:
                    ...
                    vp_flag = logs.add_parameter(vp_flag, filename, the_type, check_header, http_request_method,
                    ...
                    if settings.ENUMERATION_DONE == True:
                        ...
                        while True:
                            if not menu.options.batch:
                                ...
                                enumerate_again = input(settings.print_question_msg(question_msg))
                                ...
                                while True:
                                    try:
                                        ...
                                        if cmd.lower() in settings.SHELL_OPTIONS:
                                            os_shell_option = checks.check_os_shell_options(cmd.lower(), technique,
                                                                                            go_back, no_result)
                                            go_back, go_back_again = check_options(url, cmd, cve, check_header,
