...
def user_agent_header():
    ...
    if menu.options.mobile:
        if ((
                    menu.options.agent != settings.DEFAULT_USER_AGENT) and not menu.options.requestfile) or menu.options.random_agent:
    ...
    if menu.options.random_agent:
        if ((
                    menu.options.agent != settings.DEFAULT_USER_AGENT) and not menu.options.requestfile) or menu.options.mobile:
...
try:
    ...
    # if menu.options.output_file:
    #     output_file = menu.options.output_file
    ...
    for os_checks_num in range(0, int(settings.OS_CHECKS_NUM)):
        ...
        if not any((menu.options.url, menu.options.logfile, menu.options.bulkfile, \
                    ...
                    menu.options.update, menu.options.list_tampers, menu.options.purge,
                    menu.options.noncore_dependencies)):
        ...
        if menu.options.wizard:
            if not menu.options.url:
                while True:
                    ...
                    menu.options.url = input(settings.print_question_msg(question_msg))
            ...
            if not menu.options.data:
                ...
                menu.options.data = input(settings.print_question_msg(question_msg))
