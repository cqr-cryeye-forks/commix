...
def vuln_GET_param(url):
    ...
    if re.search(re.escape(settings.PARAMETER_DELIMITER) + r"(.*)=[^*\s\\]*(?:\\.[^*\s\\]*)*" + re.escape(
            settings.INJECT_TAG), url) or re.search(
            r"\?(.*)=[^*\s\\]*(?:\\.[^*\s\\]*)*" + re.escape(settings.INJECT_TAG), url):
...
def specify_cookie_parameter(cookie):
    ...
    if re.search(r"" + re.escape(settings.COOKIE_DELIMITER) + r"(.*)=[^*(\\/)]*(?:\\.[^*(\\/)]*)*" + re.escape(
            settings.INJECT_TAG), cookie) or \
            re.search(r"(.*)=[^*(\\/)]*(?:\\.[^*(\\/)]*)*" + re.escape(settings.INJECT_TAG), cookie):
